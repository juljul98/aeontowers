...
if ( strlen( $background ) > 0 ) {
    $bg_color_style = ' style="background-color: ' . $background . '";';
}

// Video Fallback Background Image
$video_bg_fallback = wp_get_attachment_image_src( $video_bg_fallback, 'full' );
$video_bg_fallback_img = '';
if ( strlen( $video_bg_fallback[0] ) > 0 ) {
    $video_bg_fallback_img = ' style="background-image: url(' . esc_url( $video_bg_fallback[0] ) . '); background-size: cover; background-position: 50%;"';
}


$output .= '<div class="d-section"' . $bg_color_style . $video_bg_fallback_img . '>';
